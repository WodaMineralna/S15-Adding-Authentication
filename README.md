...
- Hashing and verifying passwords with `bcryptjs`
- Protecting routes with authentication checks (only accessible to logged-in users)
- Using `csurf` middleware to add CSRF protection
- Providing user error feedback with flash messages using `connect-flash`
...
## 1) Clone the repo
```bash
git clone https://github.com/S15-Adding-Authentication
cd ./S15-Adding-Authentication
```
